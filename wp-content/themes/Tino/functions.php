...
add_action( 'widgets_init', 'register_bsi_widgets' );
function register_bsi_widgets(){
	...
	register_sidebar( array(
		'name'          => 'Main Menu Column 1',
		'id'            => "main_menu1",
		'description'   => '',
		'class'         => '',
		'before_widget' => '<div id="%1$s" class="main-menu-widget %2$s">',
		'after_widget'  => "</div>\n",
		'before_title'  => '<div class="main-menu-title h3">',
		'after_title'   => "</div>\n",
	) );
	register_sidebar( array(
		'name'          => 'Main Menu Column 2',
		'id'            => "main_menu2",
		'description'   => '',
		'class'         => '',
		'before_widget' => '<div id="%1$s" class="main-menu-widget %2$s">',
	...
	register_sidebar( array(
		'name'          => 'Main Menu Column 4 (Mobile)',
		'id'            => "main_menu5",
		'description'   => '',
		'class'         => '',
		'before_widget' => '<div id="%1$s" class="main-menu-widget %2$s">',
...
if (!function_exists('pagination')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function pagination() { // функция вывода пагинации
		...
		$links = paginate_links(array( // вывод пагинации с опциями ниже
			...
			'prev_text'    => 'Next', // текст назад
	    	'next_text'    => 'Prev', // текст вперед
...
//		wp_enqueue_script('animLib', 'https://cdnjs.cloudflare.com/ajax/libs/bodymovin/5.5.3/lottie.js','','',true);
	    wp_enqueue_script('main', get_template_directory_uri().'/assets/js/script.min.js','','',true);
	}
}
...
if (!function_exists('add_styles')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_styles() { // добавление стилей
	    if(is_admin()) return false; // если мы в админке - ничего не делаем
		wp_enqueue_style( 'main-css', get_template_directory_uri().'/assets/css/main.min.css' ); // основные стили шаблона
		wp_enqueue_style( 'icon-fonts', get_template_directory_uri().'/assets/fonts/icon-fonts/style.css' ); // основные стили шаблона
		wp_enqueue_style( 'theme', get_template_directory_uri().'/style.css' ); // основные стили шаблона
	}
}
...
if ( function_exists( 'add_image_size' ) ) {
	add_image_size( 'hero-size', 9999, 1080 );
	add_image_size( 'blog-thumb', 365, 215, true );
}
