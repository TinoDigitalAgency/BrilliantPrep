...
class ExtensionsController extends AbstractController
{
    ...
    /**
     * Determines whether the given extension is excluded.
     *
     * @since x.y.z
     *
     ...
     * @return bool
     */
    private function isExcludedExtension(AbstractExtension $extension): bool
    {
        $excluded = ArrayHelper::get(Configuration::get('mwc_extensions.excluded'), $extension->getSlug());

        if (!is_array($excluded)) {
            return (bool) $excluded;
        }

        $countries = ArrayHelper::get($excluded, 'countries', []);
        $currencies = ArrayHelper::get($excluded, 'currencies', []);

        return ArrayHelper::contains($countries, WooCommerceRepository::getBaseCountry()) &&
            ArrayHelper::contains($currencies, WooCommerceRepository::getCurrency());
    }
    ...
    private function getManagedExtensions() : array
    {
        return ArrayHelper::where(
            ...
            function ($extension) {
                return !$this->isExcludedExtension($extension);
            },
            false
        );
    }
