...
class Settings extends WC_Settings_Page {
	...
	public function load_styles_scripts( $hook_suffix ) {
		...
		$is_settings_screen = $this->plugin->is_plugin_settings();

		if ( $is_settings_screen || ( $post_type && in_array( $post_type, [ 'product', 'shop_order' ], true ) && in_array( $hook_suffix, [ 'edit.php', 'post.php', 'post-new.php' ], true ) ) ) {

			if ( $is_settings_screen && ( $background_job = $this->plugin->get_previous_orders_handler_instance()->get_job() ) ) {
			...
			wp_enqueue_script( 'wc-cog-admin', $this->plugin->get_plugin_url() . '/assets/js/admin/wc-cog-admin.min.js', $dependencies, WC_COG::VERSION );
