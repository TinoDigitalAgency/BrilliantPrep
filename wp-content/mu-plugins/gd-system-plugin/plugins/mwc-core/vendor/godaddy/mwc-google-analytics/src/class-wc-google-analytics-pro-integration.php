...
use Exception;
use GoDaddy\WordPress\MWC\GoogleAnalytics\API\Management_API;
use GoDaddy\WordPress\MWC\GoogleAnalytics\API\WC_Google_Analytics_Pro_Measurement_Protocol_API;
...
class WC_Google_Analytics_Pro_Integration extends SV_WC_Tracking_Integration {
	...
	/** @var string URL to Google Analytics Authentication proxy */
	...
	public function __construct() {
		...
		parent::__construct(
			...
			__( 'Google Analytics', 'woocommerce-google-analytics-pro' ),
			__( 'Measure your site\'s success using enhanced eCommerce tracking via Google Analytics.', 'woocommerce-google-analytics-pro' )
		);
	...
	public function load_email_tracking() {
		$this->email_tracking = wc_google_analytics_pro()->load_class( '/src/class-wc-google-analytics-pro-email-tracking.php', '\GoDaddy\WordPress\MWC\GoogleAnalytics\WC_Google_Analytics_Pro_Email_Tracking' );
	}
...
<!-- end Google Analytics -->
	...
	public function get_management_api() {
		...
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-google-analytics-pro-management-api.php' );
		// account management API request
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-google-analytics-pro-management-api-request.php' );
		// account management API responses
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/abstract-wc-google-analytics-pro-management-api-response.php' );
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-google-analytics-pro-management-api-account-summaries-response.php' );
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-google-analytics-pro-management-api-profiles-response.php' );
		...
		return $this->management_api = new Management_API( $token->access_token );
	}
	...
	public function get_measurement_protocol_api() {
		...
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-google-analytics-pro-measurement-protocol-api.php' );
		// measurement protocol API request
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-google-analytics-pro-measurement-protocol-api-request.php' );
		// measurement protocol API response
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-google-analytics-pro-measurement-protocol-api-response.php' );
	...
	public function init_form_fields() {
		...
		if ( $this->is_connected() ) {
			...
			$form_fields = array_merge(
				...
				/**
				 * Parent integration fields (includes Event Names fields)
				 * @see SV_WC_Tracking_Integration::init_form_fields()
				 */
				$this->form_fields,
	...
	protected function get_auth_fields() {
		...
		$auth_button_text = $this->get_access_token() ? esc_html__( 'Re-authenticate with your Google account', 'woocommerce-google-analytics-pro' ) : esc_html__( 'Connect your Google account', 'woocommerce-google-analytics-pro' );
	 ...
	 */
	protected function get_checkout_funnel_fields() {
	...
	/**
	 * Tracks when 404 error occurs.
	 *
	 * @since 1.11.0.dev-1
	 */
	public function track_404_error() {

		if ( is_404() ) {
			$this->js_record_event( $this->event_name['404_error'], [
				'eventCategory' => $this->event_name['404_error'],
	...
	public function authenticate() {
		...
		// trigger an action when a Google account is connected
		do_action( 'mwc_google_analytics_connected' );
