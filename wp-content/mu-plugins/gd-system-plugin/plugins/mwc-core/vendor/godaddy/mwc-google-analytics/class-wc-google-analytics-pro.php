	...
	public function add_admin_notices() {
		...
		if ( ! $integration->is_connected() ) {
			...
			if ( $this->is_plugin_settings() ) {
				...
				$notice = sprintf(
					/* translators: Placeholders: %1$s - <strong> tag, %2$s - </strong> tag, %3$s - <a> tag, %4$s - </a> tag */
					__( '%1$sGoogle Analytics is almost ready!%2$s To get started, please ​%3$sconnect to Google Analytics%4$s.', 'woocommerce-google-analytics-pro' ),
					'<strong>',
					'</strong>',
		...
		if ( $this->is_plugin_settings() && $this->is_plugin_active( 'woocommerce-google-analytics-integration.php' ) ) {
			...
			$this->get_admin_notice_handler()->add_admin_notice(
				sprintf(
					...
					__( '%1$sDeactivate Google Analytics%2$s before connecting here to avoid tracking events multiple times.', 'woocommerce-google-analytics-pro' ),
